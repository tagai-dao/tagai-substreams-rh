...
CREATE TABLE IF NOT EXISTS token_balances (
    ...
    created_at BIGINT,
    last_balance_update_time BIGINT NOT NULL DEFAULT 0,
    age NUMERIC(78, 0) NOT NULL DEFAULT 0,
    balance_source TEXT NOT NULL DEFAULT 'substreams',
    balance_rank INTEGER,
    refreshed_at TIMESTAMPTZ,
    PRIMARY KEY (token, holder)
);

-- Keep schema.sql safe for databases created before the Blockscout holder
-- refresh path was introduced.
ALTER TABLE token_balances
    ADD COLUMN IF NOT EXISTS balance_source TEXT NOT NULL DEFAULT 'substreams';
ALTER TABLE token_balances
    ADD COLUMN IF NOT EXISTS balance_rank INTEGER;
ALTER TABLE token_balances
    ADD COLUMN IF NOT EXISTS refreshed_at TIMESTAMPTZ;

CREATE UNIQUE INDEX IF NOT EXISTS token_balances_entity_index_idx
    ON token_balances (entity_index) WHERE entity_index > 0;

CREATE INDEX IF NOT EXISTS token_balances_holder_amount_idx
    ON token_balances (holder, amount DESC);

CREATE INDEX IF NOT EXISTS token_balances_token_amount_idx
    ON token_balances (token, amount DESC);

CREATE INDEX IF NOT EXISTS token_balances_token_rank_idx
    ON token_balances (token, balance_rank)
    WHERE balance_rank IS NOT NULL;

-- Trade events only mark a token dirty. A separate RH Blockscout worker claims
-- due rows, refreshes at most once per ten seconds while trades continue, and
-- schedules a four-hour fallback refresh after every successful snapshot.
-- The worker clears `dirty` optimistically using last_trade_entity_index so a
-- trade arriving during an API request cannot be lost.
CREATE TABLE IF NOT EXISTS token_holder_refresh_state (
    token TEXT PRIMARY KEY,
    dirty BOOLEAN NOT NULL DEFAULT TRUE,
    last_trade_entity_index BIGINT NOT NULL DEFAULT 0,
    last_trade_block BIGINT NOT NULL DEFAULT 0,
    last_trade_timestamp BIGINT NOT NULL DEFAULT 0,
    last_refresh_started_at TIMESTAMPTZ,
    last_refresh_succeeded_at TIMESTAMPTZ,
    next_periodic_refresh_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    lease_owner TEXT,
    lease_until TIMESTAMPTZ,
    holders_returned INTEGER,
    response_truncated BOOLEAN NOT NULL DEFAULT FALSE,
    consecutive_failures INTEGER NOT NULL DEFAULT 0,
    last_error TEXT,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CHECK (holders_returned IS NULL OR holders_returned >= 0),
    CHECK (consecutive_failures >= 0)
);

CREATE INDEX IF NOT EXISTS token_holder_refresh_state_schedule_idx
    ON token_holder_refresh_state
       (dirty DESC, next_periodic_refresh_at, last_refresh_succeeded_at);

CREATE INDEX IF NOT EXISTS token_holder_refresh_state_lease_idx
